...
def _setup_with_manual_positions() -> EnergyStationsScenario:
    """Create a scenario with known positions for deterministic tests."""
    s = EnergyStationsScenario(seed=1, player_ids=["p1", "p2"])
    s.setup()
    return s


def _place_bot_adjacent_to_station(s: EnergyStationsScenario, pid: str) -> tuple[int, int]:
...
def test_gather_adjacent_increases_energy():
    ...
    initial_caps = dict(s._station_capacities)
    ...
    for pos, cap in initial_caps.items():
        if (cap - s._gather_rate) > 0 or pos not in s._station_capacities:
            pass
    # At least one station decreased in capacity
    total_before = sum(initial_caps.values())
    total_after = sum(s._station_capacities.values())
...
def test_gather_non_adjacent_is_noop():
    ...
    pid = "p1"
    assert s._map is not None
    # Place bot far from any station
    ...
    initial_energy = s._energy[pid]
    total_before = sum(s._station_capacities.values())
    ...
    assert sum(s._station_capacities.values()) == total_before
