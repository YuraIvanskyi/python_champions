"""Test station depletion: pool at 0 capacity becomes EMPTY; is_finished on full depletion."""
...
def _make_scenario_one_pool() -> tuple[ManaPoolsScenario, tuple[int, int]]:
    """Create a scenario with a single pool of low capacity next to p1."""
    s = ManaPoolsScenario(seed=3, player_ids=["p1", "p2"])
    s.setup()
    ...
    s._pool_capacities.clear()

    # Find a clear cell to place a pool and bot adjacent to it
    for y in range(2, s._map.height - 2):
        for x in range(2, s._map.width - 2):
            if (
                ...
                s._map.set_tile(x, y, TileType.POOL)
                s._pool_capacities[(x, y)] = s._gather_rate  # capacity for exactly 1 gather
                s._positions["p1"] = (x + 1, y)
                s._positions["p2"] = (0, 0)  # far away
                return s, (x, y)

    raise RuntimeError("Could not place test pool")


def test_pool_becomes_empty_on_depletion():
    s, (sx, sy) = _make_scenario_one_pool()
    assert s._map is not None
...
def test_partial_depletion_does_not_finish_game():
    """Deplete exactly one of multiple pools; game should still be running."""
    s = ManaPoolsScenario(seed=5, player_ids=["p1", "p2"])
    s.setup()
    assert s._map is not None
    initial_count = len(s._pool_capacities)
    assert initial_count > 1

    # Pick any pool and reduce its capacity to exactly gather_rate.
    # Use _make_scenario_one_pool logic: find a pool whose adjacent cell
    # is EMPTY and NOT adjacent to any other pool (prevents gather redirecting).
    target_station = None
    bot_pos = None
    from scenarios.mana_pools.game import ORTHOGONAL_DELTAS
    for (sx, sy) in list(s._pool_capacities.keys()):
        for dx, dy in ORTHOGONAL_DELTAS:
            ...
            other_adj = [
                ...
                and s._map.get_tile(nx, ny) is TileType.POOL
                and (nx, ny) != (sx, sy)
            ]
            if not other_adj:
    ...
    if target_station is None:
        pytest.skip("Could not find isolated pool in this seed")

    sx, sy = target_station
    # Set only this pool to capacity = gather_rate so one gather depletes it
    s._pool_capacities[(sx, sy)] = s._gather_rate
    s._positions["p1"] = bot_pos
    # Put p2 far from any pool
        ...
        for x in range(s._map.width):
            if (
                ...
                and not any(
                    ...
                    and s._map.get_tile(x + ndx, y + ndy) is TileType.POOL
...
def test_is_finished_on_max_turns():
    s = ManaPoolsScenario(seed=1, player_ids=["p1", "p2"], max_turns=3)
    s.setup()
    for _ in range(3):
